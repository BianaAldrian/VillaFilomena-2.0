...
public class Room_Adapter extends RecyclerView.Adapter<Room_Adapter.ViewHolder> {
    ...
    private void deleteImage(String id) {
        String url = "http://" + ipAddress + "/VillaFilomena/manager_dir/delete/manager_delete_room.php";
        RequestQueue requestQueue = Volley.newRequestQueue(context);
        StringRequest stringRequest = new StringRequest(Request.Method.POST, url,
                response -> {
                    if (response.equals("success")) {
                        Toast.makeText(context, "Room removed", Toast.LENGTH_SHORT).show();
                        notifyDataSetChanged();
                    } else if (response.equals("failed")) {
                        Toast.makeText(context, "Room can't be removed", Toast.LENGTH_SHORT).show();
                    }
                },
